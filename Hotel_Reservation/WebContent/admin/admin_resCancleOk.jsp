...
<div><jsp:include page="../main/header2.jsp"/></div>
<div class="main">
<nav>
 ...
 <!-- Root -->
	<%
		...
		if(request.getParameter("rsno")!=null){
			...
			if(re==1){
				int pre = pdb.updateRsno(rsno); //예약테이블에서 삭제 위해서 cancle 부분 예약 번호 null 로 업데이트 시키기
	...
	<% 			
		...
		int start = (cPage -1)*5;
			...
			ArrayList<PayBean> apb = pdb.adminCancleOk(start);
	%>
...
%>
					<li class="page-item disabled">
					<a class="page-link" href="admin_resCancleOk.jsp?page=<%=startPage-1 %>" aria-label="Previous">
       				<span aria-hidden="true">&laquo;</span>
        			<span class="sr-only">Previous</span>	
     			 	</a>
     			 	</li>		
...
%>
					<li class="page-item">
					<a class="page-link" href="admin_resCancleOk.jsp?page=<%=i %>">
					<%= i %></a></li>
<% 	
					}
%>					
...
%>
						<li class="page-item disabled">
						<a class="page-link" href="admin_resCancleOk.jsp?page=<%= endPage+1 %>" aria-label="Next">
        				<span aria-hidden="true">&raquo;</span>
        				<span class="sr-only">Next</span>
      					</a>
						</li>
